...
*/
func (this *AttachmentHeader) ParseAttachmentHeader(contents string) error {
	var key string

	headerBodySplit := strings.Split(contents, "\r\n\r\n")
	if len(headerBodySplit) < 2 {
		log.Println("ERROR - ", messages.ERROR_INVALID_ATTACHMENT_BLOCK)
		return errors.New(messages.ERROR_INVALID_ATTACHMENT_BLOCK)
	}

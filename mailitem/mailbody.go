...
import (
	...
	"github.com/adampresley/mailslurper/mailheader"
...
*/
func (this *MailBody) ParseMailBody(contents string, boundary string) error {
	/*
	...
	if len(boundary) <= 0 {
		...
		for index = 0; index < len(bodyParts); index++ {
			...
			header := &mailheader.AttachmentHeader{}
			header.ParseAttachmentHeader(strings.TrimSpace(bodyParts[index]))
			...
			case strings.Contains(header.ContentDisposition, "attachment"):
				...
				this.Attachments = append(this.Attachments, attachment)
			}
		}
	}
